                          ...
                          completion:(void (^)())completion
{
    ...
    if (sign*(aPoints.a.v[axis] - bEndPoints.a.v[axis]) > 0.0f) {
        CDLog(@"Genie Effect ERROR: The distance between %@ edge of animated view and %@ edge of %@ rect is incorrect. Animation will not be performed!", edgeDescription(edge), edgeDescription(edge), reverse ? @"star" : @"destination");
        return;
    } else if (sign*(aPoints.a.v[axis] + sign*totalSize - bEndPoints.a.v[axis]) > 0.0f) {
        CDLog(@"Genie Effect Warning: The %@ edge of animated view overlaps %@ edge of %@ rect. Glitches may occur.",edgeDescription((edge + 2) % 4), edgeDescription(edge), reverse ? @"start" : @"destination");
    }
    ...
    for (int i = 0; i < totalIter; i++) {
        ...
        __block typeof(transforms) block_transforms = transforms;
        [trs enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            [block_transforms[idx] addObject:obj];
        }];
    }
...
- (NSArray *) sliceImage: (UIImage *) image toLayersAlongAxis: (BCAxis) axis
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if(imgRef != NULL) {
            UIImage *sliceImage = [UIImage imageWithCGImage:imgRef
                                                      scale:image.scale
                        ...
                        finalRectDepth: (CGFloat) rectDepth
{
    ...
    [slices enumerateObjectsWithOptions:enumerationOptions usingBlock:^(CALayer *layer, NSUInteger idx, BOOL *stop) {
        ...
        CATransform3D transform = [block_self transformRect:layer.bounds toTrapezoid:trapezoid];
