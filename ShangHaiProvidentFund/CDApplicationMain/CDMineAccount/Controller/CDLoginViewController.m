...
- (void)viewDidLoad {
    ...
    [self.tableView addSubview:self.bottomButtonView];
}
...
- (CDBottomButtonView *)bottomButtonView{
    if(_bottomButtonView == nil){
        ...
        _bottomButtonView.frame=CGRectMake(0, 0, 160, 30);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CDNormalTextFieldConfigureItem *item=[self.loginConfigureModel.arrData cd_safeObjectAtIndex:indexPath.row];
...
- (void)controlTextDidChange:(NSNotification *)noti{
    ...
    CDNormalTextFieldConfigureItem *cellItem = [self.loginConfigureModel.arrData cd_safeObjectAtIndex:textField.indexPath.row];
...
- (void)p_login{
    ...
    for (CDNormalTextFieldConfigureItem *item in self.loginConfigureModel.arrData) {
        ...
        if ([item.paramsubmit isEqualToString:@"username"]) {
            self.userNickName=item.value;
        }
