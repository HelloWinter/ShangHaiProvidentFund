...
//            NSString *path = [[NSBundle mainBundle] pathForResource:@"UserProtocol" ofType:@"html"];
            NSString *path = [[NSBundle mainBundle] pathForResource:@"UserProtocol" ofType:@"html"];
            NSLog(@"path:%@",path);
            [weakSelf p_pushToWKWebViewControllerWithTitle:@"个人用户协议" filePath:path];
//            [weakSelf p_pushToWKWebViewControllerWithTitle:@"个人用户协议" URLString:CDURLWithAPI(@"/gjjManager/noticeByIdServlet?id=yhxy") jsCode:nil];//
        };
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    CDNormalTextFieldConfigureItem *item=[self.registConfigureModel.arrData cd_safeObjectAtIndex:indexPath.row];
...
- (void)p_controlTextDidChange:(NSNotification *)noti{
    ...
    CDNormalTextFieldConfigureItem *cellItem = [self.registConfigureModel.arrData cd_safeObjectAtIndex:textField.indexPath.row];
    cellItem.value=textField.text;
}

#pragma mark - private
- (void)p_pushToWKWebViewControllerWithTitle:(NSString *)title URLString:(NSString *)urlstr jsCode:(NSString *)jsCode{
...
- (void)p_pushToWKWebViewControllerWithTitle:(NSString *)title filePath:(NSString *)filepath{
    CDBaseWKWebViewController *webViewController=[[CDBaseWKWebViewController alloc]init];
    webViewController.title=title;
    webViewController.localFilePath=filepath;
    [self.navigationController pushViewController:webViewController animated:YES];
}
...
- (void)p_showProbilemActionSheet{
    ...
    UIAlertAction *actionQuery=[UIAlertAction actionWithTitle:@"个人公积金账号查询" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction * _Nonnull action) {
        ...
        [self p_pushToWKWebViewControllerWithTitle:@"个人公积金账号查询" URLString:@"http://m.shgjj.com/verifier/verifier/index" jsCode:str];
    }];
    [alert addAction:actionCancel];
    [alert addAction:actionProblem];
...
- (void)p_regist{
    ...
    NSString *psw=nil;
    NSString *repsw=nil;
    for (CDNormalTextFieldConfigureItem *item in self.registConfigureModel.arrData) {
